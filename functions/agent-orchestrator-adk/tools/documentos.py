...
def parse_document_pdf(document_url: str, tool_context: ToolContext) -> dict:
    ...
    # Colapsar variantes del MISMO bien (descripción contenida) para no inflar
    # items_consolidados (ej. 'EQUIPO DE FTIR' ×3 desde Bases/Acta/Contrato → 1).
    try:
        _antes = len(raw.get("items_consolidados") or [])
        raw["items_consolidados"] = _merge_item_variants(raw.get("items_consolidados") or [])
