...
interface Creada {
  ...
  pago: {
    ...
    yape: string | null;
    plin: string | null;
    transferencia: { banco: string; cci: string; titular: string } | null;
    instrucciones: string;
  };
}

export function ContribuirForm({ ubigeo, zonaNombre, precioPen, restantes }: Props) {
  ...
  if (creada) {
    return (
      <div className="rounded-2xl border border-line bg-paper p-6">
        <div className="flex items-center gap-2 text-moss"><CheckCircle2 size={18} /><span className="text-sm font-semibold">Aporte registrado</span></div>
        <h3 className="mt-2 font-serif text-2xl font-bold text-ink">{creada.codigo}</h3>
        <p className="mt-1 text-sm text-mute">
          {creada.contratos} contratos en {zonaNombre} · <strong className="text-ink">{formatPEN(creada.montoPen)}</strong>
        </p>

        <div className="mt-5 rounded-xl bg-paperDeep p-4 text-sm">
          <div className="text-[11px] uppercase tracking-wide text-mute">Cómo pagar (concepto: {creada.pago.concepto})</div>
          <ul className="mt-2 space-y-1 text-ink">
            {creada.pago.yape && <li><strong>Yape:</strong> <span className="font-mono">{creada.pago.yape}</span></li>}
            {creada.pago.plin && <li><strong>Plin:</strong> <span className="font-mono">{creada.pago.plin}</span></li>}
            {creada.pago.transferencia && (
              <li><strong>Transferencia:</strong> {creada.pago.transferencia.banco} · CCI <span className="font-mono">{creada.pago.transferencia.cci}</span> · {creada.pago.transferencia.titular}</li>
            )}
            {!creada.pago.yape && !creada.pago.plin && !creada.pago.transferencia && (
              <li className="text-mute">Te enviamos los datos de pago a tu correo en las próximas horas.</li>
            )}
          </ul>
          <p className="mt-2 text-[12px] text-mute">{creada.pago.instrucciones}</p>
        </div>

        {!subido ? (
          <div className="mt-5">
            <label className="block text-sm font-medium text-ink">Sube el comprobante (foto o PDF)</label>
            <input type="file" accept="image/*,.pdf" onChange={(e) => setComprobante(e.target.files?.[0] ?? null)} className="mt-2 block w-full text-sm text-mute file:mr-3 file:rounded-lg file:border-0 file:bg-ink file:px-3 file:py-1.5 file:text-paper" />
          ...
          <div className="mt-5 rounded-xl border border-moss/30 bg-moss/5 p-4 text-sm text-ink">
            Comprobante recibido. Validamos en menos de 48 h y te avisamos por correo. Tu comprobante de impacto
            quedará en <a href={`/impacto/${creada.codigo}`} className="font-mono underline">/impacto/{creada.codigo}</a>.
          </div>
        )}
  ...
  return (
    <form onSubmit={crear} className="rounded-2xl border border-line bg-paper p-6">
      <h3 className="font-serif text-xl font-bold text-ink">Financiar auditoría en {zonaNombre}</h3>
      ...
      <div className="mt-5">
        ...
        <p className="mt-2 text-[12px] text-mute">Recibes el código de tu aporte y los datos de pago; subes el comprobante y lo validamos en menos de 48 h.</p>
      </div>
