...
const ETIQUETA_SEVERIDAD = { alta: "Señal alta", media: "Señal media", baja: "Señal baja" } as const;
...
export function CasoLeido({ caso }: { caso: CasoPortada }) {
  ...
  return (
    ...
    >
      <div className="caso-escena container-page relative max-w-[1400px] py-16 lg:flex lg:min-h-[calc(100dvh-4rem)] lg:flex-col lg:justify-center lg:py-10">
        <div className="max-w-3xl">
          <h2 id="caso-titulo" className="font-serif text-4xl font-bold leading-[1.05] tracking-tight sm:text-5xl">
            Así lee Vigía un contrato real.
          </h2>
          <p className="mt-3 max-w-[58ch] text-base leading-relaxed text-paper/70">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-start lg:gap-14">
          ...
          <article className="rounded-3xl bg-paper p-5 text-ink shadow-dialog sm:p-7">
            ...
            <h3 className="mt-3 text-lg font-semibold leading-snug text-ink">{caso.entidad}</h3>
            ...
            <div className="capa mt-5 rounded-2xl bg-crimson-soft px-4 py-3.5">
              <p className="flex items-center gap-2 text-[12px] font-semibold text-crimsonTexto">
                <Search size={13} aria-hidden /> Lo que encontró Vigía
                <span className="rounded-full bg-paper px-2 py-0.5 text-[11px] font-semibold">
                  {ETIQUETA_SEVERIDAD[caso.severidad]}
                </span>
              </p>
            ...
            {/* Capa 2: la norma */}
            <p className="capa mt-3 flex items-start gap-2 rounded-2xl bg-heroViolet-soft px-4 py-3 text-[13px] leading-relaxed text-ink">
              <Scale size={14} className="mt-0.5 shrink-0 text-heroViolet" aria-hidden />
              <span>
                <span className="font-semibold text-heroViolet">La norma que cita: </span>
                {caso.norma}
              </span>
            </p>
            ...
            <div className="capa mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-line pt-4 text-[13px]">
              {caso.fuenteUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 font-semibold text-heroViolet underline-offset-2 hover:underline"
                >
              ...
              {caso.otrasSenales > 0 && (
                <span className="text-mute">
                  Este contrato trae {caso.otrasSenales} {caso.otrasSenales === 1 ? "señal más" : "señales más"}.
                </span>
              )}
            </div>
          </article>
          ...
          <ol className="space-y-6 lg:pt-2">
            {PASOS.map(({ Icono, t, d }, i) => (
              <li key={t} className="paso flex gap-4">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-paper/20 font-mono text-sm text-paper/80">
                  {i + 1}
                </span>
                <div>
                  <h4 className="flex items-center gap-2 text-lg font-semibold text-paper">
                    <Icono size={16} className="text-heroGreen" aria-hidden />
                    {t}
                  </h4>
                  <p className="mt-1 max-w-[42ch] text-[15px] leading-relaxed text-paper/70">{d}</p>
                </div>
              </li>
            ))}
          </ol>
