...
export function FirmantesYAdjudicacionSection({
  ...
  // El rol de un firmante es un dato, no una severidad: todos en la misma tinta neutra (antes el
  // representante iba en rojo de error y quien aprobaba en ámbar de señal media).
  const rolBadge = (_r: string) => "bg-paperDeep text-inkSoft";
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-line bg-paper">
      <div className="border-b border-line bg-paperSoft px-5 py-3">
        <h2 className="font-display text-xl font-bold text-ink">Firmantes y motivos de adjudicación</h2>
        ...
        {(lugarFecha?.lugar || lugarFecha?.fecha) && (
          <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-mute">
            ...
            {lugarFecha?.fecha && (
              <span className="inline-flex items-center gap-1">
                <Calendar size={11} aria-hidden /> {fechaDossier(lugarFecha.fecha)}
              </span>
            )}
            {lugarFecha?.hora && (
              <span className="inline-flex items-center gap-1">
            ...
            <ul className="mt-2 space-y-2">
              {firmantes.map((f: any, i: number) => (
                <li key={i} className="rounded-md bg-paper px-3 py-2">
                  ...
                  {f.entidad && <div className="text-[11px] text-mute">{f.entidad}</div>}
                  <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-mute">
                    ...
                    {f.fecha_firma && (
                      <span className="inline-flex items-center gap-1">
                        <Calendar size={11} aria-hidden /> {fechaDossier(f.fecha_firma, true)}
                      </span>
                    )}
                  </div>
                </li>
              ...
              {comite.map((m: any, i: number) => (
                <li key={i} className="rounded-md bg-paper px-3 py-2">
                  <div className="flex flex-wrap items-baseline gap-2">
                    ...
                    {m.rol && (
                      <span className={cn("rounded-full px-2 py-0.5 text-[11px] font-semibold", rolBadge(m.rol))}>
                  ...
                  {m.certificacion_sican && (
                    <div className="mt-1 inline-flex items-center gap-1 rounded-full bg-paperDeep px-2 py-0.5 text-[11px] text-inkSoft">
      ...
      {motivos.length > 0 && (
        <div className="border-t border-line px-5 py-5">
          <h3 className="mb-3 flex items-center gap-1.5 text-[12px] font-semibold text-ink">
            <Award size={12} className="text-mute" aria-hidden />
              ...
              return (
                <li key={i} className="rounded-lg border border-line bg-paperSoft p-3">
                  <div className="flex flex-wrap items-baseline gap-2">
                    ...
                    {m.posicion_ranking && (
                      <span className="ml-auto rounded-full bg-paperDeep px-2 py-0.5 text-[11px] font-semibold text-inkSoft">
                  ...
                  {m.evidencia && normalizada(m.evidencia) && (
                    <div className="mt-2 border-t border-line/60 pt-2 text-[11px] text-inkSoft">
                      <div className="mb-0.5 text-[11px] font-medium text-mute">Lo que dice el acta</div>
              ...
              <li key={i} className="rounded-md bg-paper px-3 py-2">
                ...
                {c.tipo_relacion && !SIN_VINCULO.has(String(c.tipo_relacion).toLowerCase()) && (
                  <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[12px]">
                    <Severidad bandera={severidadDe(c)} formato="linea" />
                    <span className="text-ink">{String(c.tipo_relacion).replace(/_/g, " ")}</span>
                  </div>
                )}
