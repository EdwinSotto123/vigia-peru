...
function PanelResumen({ result, resumenEjecutivo, banderasArr, noVerificables, corrida, onLeerDictamen }: PanelDossierProps) {
    ...
    <div className="space-y-4">
      <div id="senales" className="scroll-mt-24">
        {banderasArr.length > 0 || noVerificables.length > 0 ? (
          ...
          <AvisoSeccion titulo="El análisis terminó sin señales">
            Se evaluaron las reglas de contratación y se escribió el dictamen. Ninguna regla disparó una señal. No es un
      ...
      {resumenEjecutivo && (
        <SeccionSegura nombre="el resumen ejecutivo">
          <div className="rounded-2xl border border-line bg-paperSoft p-4 sm:p-5">
            ...
            <p className="mt-1.5 line-clamp-4 max-w-[68ch] text-sm leading-relaxed text-inkSoft">{resumenEjecutivo}</p>
            <button
              type="button"
              onClick={onLeerDictamen}
              className="mt-3 inline-flex min-h-[32px] items-center gap-1.5 rounded-full bg-granate px-3.5 py-1.5 text-[13px] font-semibold text-paper transition-colors hover:bg-granate-deep"
            >
