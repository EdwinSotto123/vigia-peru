...
gcloud run deploy vigia-peru-api \
  ...
  --update-env-vars "^|^PGHOST=/cloudsql/${SQL_CONNECTION}|PGUSER=postgres|PGDATABASE=vigia|FIREBASE_PROJECT_ID=${FIREBASE_PROJECT_ID}|GCS_PROJECT_ID=${PROJECT_ID}|GCS_BUCKET_DOCUMENTOS=${BUCKET_DOCUMENTOS}|GCS_BUCKET_REPORTES=${BUCKET_REPORTES}|ALLOWED_ORIGINS=${ALLOWED_ORIGINS}|LOCAL_DOWNLOADER_URL=${LOCAL_DOWNLOADER_URL:-http://149.104.66.211:8080}" \
