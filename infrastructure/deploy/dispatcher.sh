...
gcloud run jobs deploy vigia-dispatcher --source . --region "$REGION" \
  ...
  --set-env-vars "AGENT_URL=${AGENT_URL},AGENT_URL_BIENES=${AGENT_URL_BIENES},AGENT_URL_SERVICIOS=${AGENT_URL_SERVICIOS},AGENT_URL_OBRAS=${AGENT_URL_OBRAS},AGENT_URL_OTROS=${AGENT_URL_OTROS},PGHOST=/cloudsql/${SQL_CONNECTION},PGUSER=postgres,PGDATABASE=vigia,DISPATCHER_PARALLEL=${DISPATCHER_PARALLEL:-1},DISPATCHER_MAX_MINUTES=25,DISPATCHER_TASK_TIMEOUT_S=7200,DISPATCHER_ANALISIS_MAX_S=3600" \
