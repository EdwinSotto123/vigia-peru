"""Construye document_parser_agent."""
...
_kw = {}
_planner = build_planner("document_parser_agent", config.MODEL, getattr(config, "THINKING", None))
if _planner is not None:
    _kw["planner"] = _planner
_gcc = build_generate_config("document_parser_agent", getattr(config, "TEMPERATURE", None),
...
if config.STATE_INJECTIONS:
    document_parser_agent.instruction = make_state_aware_instruction(
        document_parser_agent.instruction, config.STATE_INJECTIONS,
    )
