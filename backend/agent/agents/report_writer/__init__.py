...
_kw = {}
_planner = build_planner("report_writer_agent", config.MODEL, getattr(config, "THINKING", None))
if _planner is not None:
    _kw["planner"] = _planner
_gcc = build_generate_config("report_writer_agent", getattr(config, "TEMPERATURE", None),
...
if config.STATE_INJECTIONS:
    report_writer_agent.instruction = make_state_aware_instruction(
        report_writer_agent.instruction, config.STATE_INJECTIONS,
    )
