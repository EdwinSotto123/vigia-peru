...
_kw = {}
_planner = build_planner("web_research_agent", config.MODEL, getattr(config, "THINKING", None))
if _planner is not None:
    _kw["planner"] = _planner
_gcc = build_generate_config("web_research_agent", getattr(config, "TEMPERATURE", None),
...
if config.STATE_INJECTIONS:
    web_research_agent.instruction = make_state_aware_instruction(
        web_research_agent.instruction, config.STATE_INJECTIONS,
    )
